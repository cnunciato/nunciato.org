...
const userAgent =
    process.env.GITHUB_USER_AGENT || "Christian's Media-Processor Uploader Service Thing";
...
if (source) {
    if (source.startsWith("s3://")) {
        ...
        processFiles("./workdir", true)
            .then(results => {
                results.forEach(item => {
                    ...
                    switch (fileType) {
                        case "video":
                            relativePath = `${repoContentPath}/videos/${item.filename}.md`;
                            ...
                            break;
                        case "photo":
                            relativePath = `${repoContentPath}/photos/${item.filename}.md`;
                            ...
                            break;
                        case "sound":
                            relativePath = `${repoContentPath}/sounds/${item.filename}.md`;
                    ...
                    if (!frontmatter || !relativePath) {
                        throw new Error(
                            `💥 Unable to derive frontmatter or relativePath from processFiles result: ${toJSON(results)}.`,
                        );
                    }

                    // Submit the new content file to GitHub.
                    submitToGitHub({ path: relativePath, frontmatter, content: "" }).then(() => {
        ...
        // Just process and write the resulting JSON or YAML to stdout.
...
function submitToGitHub(file: ContentFile): Promise<any> {
    console.log(
        `➡️ Sending to GitHub: ${file.path}, ${toJSON(file.frontmatter)}, ${file.content}...`,
    );
...
function processFiles(sourceDir: string, useGPS: boolean): Promise<(ProcessingResult | null)[]> {
    ...
    return new Promise<ProcessingResult[]>((resolve, reject) => {
        return Promise.all(files.map(file => exiftool.read(file)))
            .then(results => {
                for (let i = 0; i < results.length; i++) {
                    ...
                    const filename = tagsToFilename(tags);
                    ...
                    if (!type) {
                        console.error(
                            `🤔 Couldn't derive a submission type for ${file}. Skipping.`,
                        );
                        continue;
                    }
                    ...
                    if (type === "photo") {
                        ...
                        console.error(
                            `🤔 Unprocessable type for ${file} (It was ${type}). Skipping.`,
                        );
                        continue;
                    }
                    ...
                    if (type === "video") {
                        ...
                        execSync(
                            `ffmpeg -i "${file}" -vf "fade=in:0:30,fade=out:st=${duration - 1}:d=1,scale=${largeWidth}:-1" -af "afade=in:st=0:d=1,afade=out:st=${duration - 1}:d=1" -vcodec h264 -acodec aac -strict -2 "${videoPath}/${mediaFilename}"`,
                        );
...
function getMediaDuration(path: string): number {
    return parseInt(
        execSync(
            `ffprobe -i "${path}" -show_entries stream=codec_type,duration -of compact=p=0:nk=1 | head -1`,
        )
            .toString()
            .trim()
            .split("|")
...
function makeResizedImage(source: string, destination: string, width: number) {
    execSync(
        `ffmpeg -i "${source}" -vf "scale=${width}:-1" -pix_fmt yuvj422p -q:v 4 -y "${destination}"`,
    );
}
...
function makeVideoThumbnail(source: string, destination: string, width: number) {
    execSync(
        `ffmpeg -i "${source}" -vf "select=gte(n\\,100),scale=${width}:-1" -vframes 1 "${destination}.jpg"`,
    );
}
...
function makeSound(source: string, destination: string, duration: number) {
    execSync(
        `ffmpeg -i "${source}" -vf "fade=in:0:30,fade=out:st=${duration - 1}:d=1" -af "afade=in:st=0:d=1,afade=out:st=${duration - 1}:d=1" -acodec aac -strict -2 "${destination}"`,
    );
}
...
function tagsToCreated(tags: Tags): ExifDateTime | undefined {
    ...
    return (tags.DateTimeCreated ||
        tags.DateCreated ||
        tags.DateTimeOriginal ||
        // tags["CreationDate"] || // Not in the type def. Do I need it?
        tags.MetadataDate ||
...
function tagsToFilename(tags: Tags): string | undefined {
    ...
    return [created.year, created.month, created.day, created.hour, created.minute, created.second]
